...
| Simple JSON test on disk | Contains the obsolete Python 3.7 interpreter name; correct it in YASE before reuse |
...
## Simple JSON test and on-disk caveat
...
The JSON round trip succeeded at runtime on 2026-07-10. However, the current
on-disk `SUB_JSON_WriteExtract.xseq` still contains:

```text
Python_37_PYTHON_AUTOMATION_INTERPRETER
```

Before reusing that sequence, change and save its `Interpreter` field as:

```text
Python_310_PYTHON_AUTOMATION_INTERPRETER
```

Do not describe this file as ready until the on-disk field has been corrected
and verified after closing YASE.
