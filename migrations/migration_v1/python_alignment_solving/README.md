...
For the current Python Automation machine, copy the legacy solver file directly
into the process-local Python working directory used by `TMPython.ini`:

```text
D:\TestMasterData\Process\Python_Automation\python_env\fixed_z_alignment_solver.py
```
...
Module      = fixed_z_alignment_solver
...
```powershell
Get-Content migrations\migration_v1\python_alignment_solving\examples\fixed_z_alignment_input.json | python migrations\migration_v1\python_alignment_solving\fixed_z_alignment_solver.py
```

To regenerate the checked-in example output shape:

```powershell
Get-Content migrations\migration_v1\python_alignment_solving\examples\fixed_z_alignment_input.json | python migrations\migration_v1\python_alignment_solving\fixed_z_alignment_solver.py | python -m json.tool
```
