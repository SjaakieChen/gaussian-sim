...
The bundle separates YASE-side files from Python/runtime files:
...
Do not copy the repository `.git`, `.pytest_cache`, `tests`, `machine_documentation`,
...
Then use `Interpreter = Python_310_ALIGNMENT_TEST` in the YASE
...
| --- | --- |
| `Interpreter = Python_310_ALIGNMENT_TEST` | Look up `[Python_310_ALIGNMENT_TEST]` in `TMPython.ini`. |
...
Input JSON  = fixed JSON or s_PythonInputJson
Result JSON = s_PythonResultJson
```
